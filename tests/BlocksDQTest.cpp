...
TEST(initialization_test, RunsAndReturnsValidData) {
    size_t n = 10;
    size_t M = 4;
    double B = 20;
...
TEST(insert_test, RunsAndReturnsValidData) {
    // Test 1: Basic insertion with unique values
    {
        constexpr size_t n = 100;
        constexpr size_t M = 4;
        constexpr double B = 1000;
        DequeueBlocks dq(n, M, B);
        ...
        for (const auto& p : pulled) {
            ...
            ASSERT_LE(p.value_, m_th_value + 1e-9);
        }
        ...
        ASSERT_LE(strictly_less_count, M);
    ...
    // Test 2: Many duplicates at the boundary
    {
        ...
        for (const auto& p : pulled) {
            ASSERT_TRUE(std::abs(p.value_ - 1.0) < 1e-9);
        }
    ...
    // Test 3: Insert with decreasing values (should update)
    {
        ...
        for (const auto& p : pulled) {
            if (p.key_->id_ == 0) {
                ASSERT_TRUE(std::abs(p.value_ - 25.0) < 1e-9);
                found = true;
                break;
            }
        }
    ...
    // Test 4: Insert in reverse order (largest to smallest)
    {
        ...
        for (int i = 0; i < M; ++i) {
            ASSERT_TRUE(std::abs(pulled[i].value_ - i * 10.0) < 1e-9);
        }
    ...
    // Test 5: Random insertions with verification
    {
        ...
        for (const auto& p : pulled) {
            ...
            else ASSERT_TRUE(false && "Pulled value greater than M-th smallest!");
        }

        // All strictly less than m_th_value must be in pulled set
        ASSERT_TRUE(pulled_strictly_less == strictly_less_count);

        // Total pulled should be M
        ASSERT_TRUE(pulled_strictly_less + pulled_equal == M);
    ...
    // Test 6: Edge case - exactly M elements total
    {
        ...
        auto [pulled, bound] = dq.pull();
        ASSERT_TRUE(pulled.size() == M);  // Should pull all elements
        ...
        for (int i = 0; i < M; ++i) {
            ASSERT_TRUE(pulled_ids.find(i) != pulled_ids.end());
        }
    ...
    // Test 7: Edge case - fewer than M elements total
    {
        ...
        auto [pulled, bound] = dq.pull();
        ASSERT_TRUE(pulled.size() == n);  // Should pull all n elements (n < M)
...
TEST(batch_prepend_test, RunsAndReturnsValidData) {
    ...
    *[DQ] Init: N: 6, M: 1, B: inf
    ...
    // Test 1: Small batch (L ≤ M)
    {
        ...
        for (int i = 0; i < 5; ++i) {
            ASSERT_TRUE(pulled_set.find({i, i * 10.0}) != pulled_set.end());
        }
    ...
    // Test 2: Large batch (L > M) - should create multiple blocks
    {
        ...
        for (const auto& p : pulled) {
            ...
            ASSERT_TRUE(found && "Pulled element not among M smallest");
        }
    ...
    // Test 3: Batch prepend after some insertions
    {
        ...
        for (int i = 0; i < M; ++i) {
            ASSERT_TRUE(std::abs(pulled[i].value_ - expected[i]) < 1e-9);
        }
    ...
    // Test 4: Batch prepend with duplicate values
    {
        ...
        ASSERT_TRUE(zero_count == 3);
        ASSERT_TRUE(ten_count == 1);
    ...
    // Test 6: Edge case - empty batch
    {
        ...
        ASSERT_TRUE(!dq.empty());  // Still has initial D1 block

    }

}

TEST(erase_test, RunsAndReturnsValidData) {
    ...
    // Test 1: Basic erase
    {
        ...
        for (const auto& p : pulled) {
            ASSERT_TRUE(std::abs(p.value_ - 20.0) > 1e-9);
        }
        ...
        for (double val : expected) {
            ASSERT_TRUE(pulled_values.find(val) != pulled_values.end());
        }
    ...
    // Test 2: Erase non-existent vertex
    {
        ...
        auto [pulled, bound] = dq.pull();
        ASSERT_TRUE(pulled.size() == 2);  // Only 2 elements total (< M)
    ...
    // Test 4: Erase causes block deletion (D1)
    {
        ...
        auto [pulled, bound] = dq.pull();
        ASSERT_TRUE(pulled.size() == 1);
        ASSERT_TRUE(pulled[0].key_->id_ == M);
        ASSERT_TRUE(std::abs(pulled[0].value_ - M * 0.1) < 1e-9);
    ...
    // Test 5: Erase from D0 (batch prepend block)
    {
        ...
        for (const auto& p : pulled) {
            ASSERT_TRUE(std::abs(p.value_ - 5.0) > 1e-9);
        }

        // Verify we have the right set
        std::set<double> expected = {0.0, 10.0, 15.0, 20.0};
        for (const auto& p : pulled) {
            ASSERT_TRUE(expected.find(p.value_) != expected.end());
        }
    ...
    // Test 6: Erase all elements one by one
    {
        ...
        ASSERT_TRUE(std::abs(bound - B) < 1e-9);
    ...
    // Test 7: Erase from middle of block (verifies swap removal works)
    {
        ...
        for (int i = 0; i < M; ++i) {
            ...
            if (i < 3) {
                ...
                ASSERT_TRUE(dq.is_present(vertices[i]->id_));
            } else if (i == M-1) {
        ...
        auto [pulled, bound] = dq.pull();
        ASSERT_TRUE(pulled.size() == M-1);  // 7 elements remaining

        // Value 30 should not be present
        for (const auto& p : pulled) {
            ASSERT_TRUE(std::abs(p.value_ - 30.0) > 1e-9);
        }
...
TEST(test_pull_bounds, RunsAndReturnsValidData) {
    ...
    ASSERT_TRUE(pulled1.size() == 4);
    ASSERT_TRUE(std::abs(bound1 - 4.0) < 1e-9);

    // Second pull: should get [4, 5, 6, 7], bound = 8.0
    auto [pulled2, bound2] = dq.pull();
    ASSERT_TRUE(pulled2.size() == 4);
    ASSERT_TRUE(std::abs(bound2 - 8.0) < 1e-9);

    // Third pull: should get [8, 9], bound = B (since ≤ M elements left)
    auto [pulled3, bound3] = dq.pull();
    ASSERT_TRUE(pulled3.size() == 2);
    ASSERT_TRUE(std::abs(bound3 - B) < 1e-9);
